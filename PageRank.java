    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < 2*numRepititions; i++) {
            ...
            if(i%2 == 0) {
                ...
                leftover = job.getCounters().findCounter(Counter.counters.DANGLINGNODE).getValue();
                size = job.getCounters().findCounter(Counter.counters.GLOBALNODE).getValue();

            } else {
                // Set up leftover and size
                job.getCounters().findCounter(Counter.counters.DANGLINGNODE).setValue(0);
